...
TimerEvent::TimerEvent(int64_t interval, CallBackFunc cb, bool periodic /*=false*/):
...
void TimerFdEvent::addTimerEvent(SP_Timer timer)
{
...
void TimerFdEvent::deleteTimerEvent(SP_Timer timer)
{
    // 标记删除即可
...
void TimerFdEvent::resetTimerFd()
{
    ...
    SP_Timer first_timer = m_pending_timers.top();
    mutex.unlock();
